...
- **Static fallback asset swap:** replace `public/logo-farah-ramadan.png` with the updated logo file the user provides during implementation (pasted into the conversation, saved to `public/`). This is the image shown when no logo is uploaded in `/admin`, and is also the file used to derive the `width`/`height` props on both `Navbar` and `Footer` — if the new file's aspect ratio differs from 821×312, update those dimensions on both components to match, keeping the same rendered heights (`h-[108px]` navbar / `h-16` footer).
